...
- `SEED_PARTNER_EMAIL` — при `pnpm prisma db seed` создаёт юзера с ролью `partner`, привязывает к первому партнёру (`PARTNERS[0]` = `monkey-grinder`) и добавляет в его команду. Юзер заводится со статусом `pending`, активируется при первом magic-link логине. Только для dev — для прод-партнёров используй админку `/admin/partners/<id>` → секция «Команда».
