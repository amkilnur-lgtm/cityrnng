...
| 3. Check-in | ✅ QR | Strava-механика снесена (2026-07). Зачёт: персональный QR (`User.checkinCode`) → сканер на точке (`ScanDevice`, Raspberry Pi, `devices/runbase-scanner/`) → `POST /integrations/checkin/scan` (device-key, идемпотентно по scanId) → `event_attendances` (source `qr_scan`, авто-approve) → баллы. Журнал `checkin_scans`. **Frontend:** QR-баннер на `/app` + карточка в `/app/profile` (fullscreen по тапу), админка `/admin/checkin` (сканеры, ключи, журнал). Дистанция в зачёте не участвует — баллы за приход |
...
## V. RSVP back on event detail page — pace stays informational (2026-05-15)

Откат части PR #77. После 2 недель использования стало понятно: убирать
RSVP с детальной страницы было ошибкой — юзер видит pace groups + карту
на `/events/[id]`, но записаться не может, надо идти на `/app` без
контекста. В итоге путаница «где смотреть, где записываться».

- `<EventRsvp>` (та же инкарнация, что и раньше) перенесена обратно на
  `/events/[id]` под текущий блок c pace groups. Гость видит read-only
  карточки + кнопку «Войти, чтобы записаться».
- `<EventLocationsDisplay>` удалена (была обёрткой EventRsvp в
  read-only режиме; теперь EventRsvp сам это умеет через `isAuthed`).
- `/app` больше не редактирует RSVP. Вместо `<NextEvent rsvp>` —
  новый компонент `<MyUpcomingRsvps>`: список «мои ближайшие записи»
  с CTA «Открыть событие». Получает данные через
  `getMyUpcomingRsvps()` (грузит `listUpcomingEvents` × `getMyInterest`
  фан-аутом — N небольшое).
- `<NextEvent>` на `/`: убран inline-RSVP-блок (`showInlineRsvp` ушёл),
  CTA сменился с «Маршрут и точка старта» на «Подробнее и записаться».
  Под заголовком появляется красная плашка «ты идёшь · точка X», если
  у юзера уже есть запись.

Темп **не выбирается** в RSVP — pace groups остаются справочной
информацией, темп решается на месте. Значит миграции БД нет, контракт
API не меняется (`paceGroupId` не вводили).
