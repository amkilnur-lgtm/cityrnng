...
export class StravaApiClient {
  ...
  async getActivity(accessToken: string, id: number | string): Promise<StravaActivity> {
    return this.getJson<StravaActivity>(`${STRAVA_BASE}/activities/${id}`, accessToken);
  }

  // ===== Push subscription (app-wide webhook registration) =====
  // App-level credentials (client_id + client_secret) auth this, NOT a user token.

  async listSubscriptions(): Promise<StravaSubscription[]> {
    const url = new URL(`${STRAVA_BASE}/push_subscriptions`);
    url.searchParams.set("client_id", this.config.get("STRAVA_CLIENT_ID", { infer: true }));
    url.searchParams.set("client_secret", this.config.get("STRAVA_CLIENT_SECRET", { infer: true }));
    const res = await fetch(url.toString(), {
      headers: { accept: "application/json", "user-agent": USER_AGENT },
    });
    if (!res.ok) {
      const detail = await safeText(res);
      this.logger.error(`Strava list subscriptions failed ${res.status}: ${detail}`);
  ...
  async createSubscription(callbackUrl: string, verifyToken: string): Promise<StravaSubscription> {
    ...
    return this.postForm<StravaSubscription>(`${STRAVA_BASE}/push_subscriptions`, body);
  }

  async deleteSubscription(id: number): Promise<void> {
    const url = new URL(`${STRAVA_BASE}/push_subscriptions/${id}`);
    url.searchParams.set("client_id", this.config.get("STRAVA_CLIENT_ID", { infer: true }));
    url.searchParams.set("client_secret", this.config.get("STRAVA_CLIENT_SECRET", { infer: true }));
    const res = await fetch(url.toString(), {
      method: "DELETE",
    ...
    if (!res.ok && res.status !== 404) {
      const detail = await safeText(res);
      this.logger.error(`Strava delete subscription ${id} failed ${res.status}: ${detail}`);
      throw new InternalServerErrorException({ code: "STRAVA_UPSTREAM_ERROR" });
    }
  }
